...
import { updateRecepcionistaPerfil } from "../../Src/Service/RecepcionService"; 

export default function EditarRecepcionistaScreen({ navigation }) {
  ...
  useEffect(() => {
    const CargarDatos = async () => {
      ...
      try {
        ...
        if (!token || role !== "recepcionista") {
          Alert.alert("Error", "Permiso denegado.");
          navigation.goBack();
          return;
        }
        
        const response = await apiConexion.get("/me/recepcionista", {
  ...
  const handleUpdate = async () => {
    ...
    try {
        ...
        const result = await updateRecepcionistaPerfil(userData);
